...
Integrates with:
- orchestrator.py (listing_matches_v2) for boolean matching
- numeric_constraints.py for constraint evaluation
- item_matchers.py for item-level matching
...
def _evaluate_items_detailed(
    ...
    for i, req_item in enumerate(required_items):
        ...
        # RANGE constraints (M-11) - includes EXACT values as [x,x]
        ...
        for key, req_range in req_ranges.items():
            ...
            if cand_range is None:
                ...
                passed = range_contains(cand_range, tuple(req_range))
                if not passed:
