...
def listing_matches_v2(A: Dict[str, Any],
    ...
    # ========================================================================
    # PRE-STEP: CURRENCY NORMALIZATION
    # Convert all currency-tagged values to USD using live exchange rates.
    # This ensures cross-currency comparisons are correct (e.g., INR vs USD).
    # Original listings are NOT mutated — deep copies are used.
    # ========================================================================
    A = _normalize_currencies_for_matching(A)
    B = _normalize_currencies_for_matching(B)
    ...
    if intent == "product" or intent == "service":
        ...
        # Now with ontology-aware exclusion checking
        if not all_required_items_match(A["items"], B["items"], implies_fn, ontology_resolver):
    ...
    # Now with ontology-aware exclusion checking
    if not match_other_to_self(A["other"], B["self"], implies_fn, ontology_resolver):
...
def _normalize_currencies_for_matching(listing: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convert all currency-tagged numeric values to USD for matching.

    Reads the _currencies companion field (preserved by the normalizer)
    and converts corresponding values in min/max/range to USD using
    live exchange rates from frankfurter.app.

    Creates a deep copy — the original listing is NOT mutated.
    If the API is unavailable, returns the listing unchanged.

    Args:
        listing: Normalized listing in OLD schema format

    Returns:
        Copy of listing with currency values converted to USD
    """
    # Check if any section has currencies before doing expensive deep copy
    has_currencies = False
    for item in listing.get("items", []):
        if item.get("_currencies"):
            has_currencies = True
            break
    if not has_currencies:
        for section in ("other", "self"):
            if listing.get(section, {}).get("_currencies"):
                has_currencies = True
                break

    if not has_currencies:
        return listing  # No currencies anywhere, skip entirely

    from src.services.external.currency_service import get_currency_service
    currency_service = get_currency_service()

    converted = deepcopy(listing)

    # Convert items
    for item in converted.get("items", []):
        _convert_constraint_currencies(item, currency_service)

    # Convert other/self
    for section in ("other", "self"):
        obj = converted.get(section, {})
        if obj:
            _convert_constraint_currencies(obj, currency_service)

    return converted


def _convert_constraint_currencies(
    constraint_obj: Dict[str, Any],
    currency_service
) -> None:
    """
    In-place convert currency-tagged values to USD on a copied dict.

    Reads _currencies to find which attributes have currency metadata,
    fetches live rates, and multiplies the corresponding values in
    min/max/range by the exchange rate.
    """
    currencies = constraint_obj.get("_currencies", {})
    if not currencies:
        return

    from src.config.constants import MATCHING_TARGET_CURRENCY
    target = MATCHING_TARGET_CURRENCY

    for attr_name, currency_code in currencies.items():
        currency_code = currency_code.upper().strip()

        if currency_code == target:
            continue

        if currency_code == "LOCAL":
            # "local" means no currency was specified — skip conversion
            continue

        rate = currency_service.get_rate(currency_code, target)
        if rate is None:
            print(f"Warning: Could not get {currency_code}->{target} rate. "
                  f"'{attr_name}' compared in original currency.")
            continue

        # Convert min
        if attr_name in constraint_obj.get("min", {}):
            constraint_obj["min"][attr_name] *= rate

        # Convert max
        if attr_name in constraint_obj.get("max", {}):
            constraint_obj["max"][attr_name] *= rate

        # Convert range
        if attr_name in constraint_obj.get("range", {}):
            r = constraint_obj["range"][attr_name]
            constraint_obj["range"][attr_name] = [r[0] * rate, r[1] * rate]
