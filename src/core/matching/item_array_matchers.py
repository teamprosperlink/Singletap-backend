...
from .item_matchers import item_matches, ImplicationFn
...
def flatten_item_values(item: Dict[str, Any]) -> Set[str]:
    ...
    - **NEW**: Extracts concept_id from ontology structures (preserved from canonicalization)
    ...
    Examples:
        ...
        # New format (ontology structures)
        flatten_item_values({
            "type": "smartphone",
            "categorical": {
                "brand": {"concept_id": "apple", "concept_root": "brand", ...},
                "condition": {"concept_id": "refurbished", "concept_root": "condition", ...}
            }
        }) → {"smartphone", "apple", "refurbished"}

    Note:
        - Numeric values in min/max/range are NOT extracted (they're constraints, not attributes)
        - Keys are NOT extracted (only values)
        - For ontology dicts, extracts concept_id instead of stringifying the dict
        - Already normalized by Phase 2.1 (lowercase, trimmed)
    """
    values = set()

    def extract_strings(obj: Any) -> None:
        """Recursively extract string values, handling ontology structures."""
        if isinstance(obj, dict):
            # Check if this is an ontology structure
            if "concept_id" in obj:
                # Extract the concept_id from ontology dict
    ...
    # Extract from categorical values (handles both old string format and new ontology format)
...
def violates_item_exclusions(
    ...
    ENHANCED: Now supports ontology-aware exclusion checking with hierarchy.
    - Handles both old format (simple strings) and new format (ontology dicts)
    - Excluding "non-veg" will catch "chicken" (chicken implies non-veg)
    - Uses concept_path and match_scope for descendant expansion
    - Excluding "used" with match_scope="include_descendants" catches "refurbished", "second hand", etc.
    ...
    Preconditions:
        ...
        - itemexclusions is array of strings OR ontology dicts
        - Candidate item has extractable string values

    Pass Condition (exclusions NOT violated):
        ∀ excl ∈ required.itemexclusions:
          excl ∉ Flatten(candidate_item) AND candidate_value does NOT imply excl

    Fail Condition (exclusions violated):
        ∃ excl ∈ required.itemexclusions:
          excl ∈ Flatten(candidate_item) OR candidate_value implies excl

    Args:
        required_item: Item from listing A (with exclusions)
        candidate_item: Item from listing B (being checked)
        ontology_resolver: Optional ontology resolver for hierarchical checking
    ...
    Examples:
        # Exact match violation (old format)
        violates_item_exclusions(
            {"itemexclusions": ["refurbished", "used"]},
            {"categorical": {"condition": "refurbished"}}
        ) → True (VIOLATED - exact match)

        # Ontology format exclusion (new format)
        violates_item_exclusions(
            {"itemexclusions": [{"concept_id": "used", "match_scope": "include_descendants", "children": ["refurbished", "second hand"]}]},
            {"categorical": {"condition": {"concept_id": "refurbished", ...}}}
        ) → True (VIOLATED - refurbished is descendant of used)

        # Hierarchical violation (with ontology resolver)
        violates_item_exclusions(
            {"itemexclusions": [{"concept_id": "non-veg", ...}]},
            {"categorical": {"protein": {"concept_id": "chicken", ...}}},
            ontology_resolver
        ) → True (VIOLATED - chicken implies non-veg)
    ...
    Notes:
        ...
        - Any overlap (exact or hierarchical) → DISJOINT requirement violated
        - Empty exclusions → no restrictions
        - Case-insensitive (Phase 2.1 normalized)
        - Backward compatible with simple string exclusions
    """
    ...
    for exclusion in required_exclusions:
        # Handle both old format (string) and new format (ontology dict)
        if isinstance(exclusion, dict) and "concept_id" in exclusion:
            # New format: ontology structure
            raw_concept = exclusion.get("concept_id", "")
            excl_concept_id = str(raw_concept).lower().strip() if raw_concept else ""
            excl_match_scope = exclusion.get("match_scope", "exact")
            excl_children = exclusion.get("metadata", {}).get("children", [])

            # Check exact match
            if excl_concept_id in candidate_values:
                return True  # VIOLATION FOUND

            # Check descendants if match_scope includes them
            if excl_match_scope == "include_descendants":
                for child in excl_children:
                    if isinstance(child, str) and child.lower().strip() in candidate_values:
                        return True  # DESCENDANT VIOLATION FOUND

            # Ontology-aware hierarchical checking
            if ontology_resolver:
                for candidate_value in candidate_values:
                    violated, reason = ontology_resolver.check_exclusion_violation(
                        excl_concept_id, candidate_value
                    )
                    if violated:
                        return True  # HIERARCHICAL VIOLATION FOUND
...
def required_item_has_match(
    ...
    for candidate_item in candidate_items:
        ...
        # Pass ontology_resolver for hierarchical exclusion checking
        if violates_item_exclusions(required_item, candidate_item, ontology_resolver):
...
def all_required_items_match(
    ...
    for required_item in required_items:
        if not required_item_has_match(required_item, candidate_items, implies_fn, ontology_resolver):
