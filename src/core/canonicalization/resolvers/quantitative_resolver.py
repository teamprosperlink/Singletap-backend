...
- "100 watts"          → Quantulum: 100 watt     → Pint base: 100 kg*m²/s³
"""

from typing import Dict, Optional
from src.services.external.quantulum_wrapper import extract_first_quantity
from src.services.external.pint_wrapper import normalize_to_base
...
class QuantitativeResolver:
    ...
    1. Extract quantity from text (Quantulum3 — any unit in the world)
    2. Normalize to SI base unit (Pint — dynamic, no hardcoded mapping)
    """

    def __init__(self):
        """Initialize quantitative resolver."""
        pass
    ...
    def resolve(self, value_str: str) -> Optional[Dict]:
        ...
        """
        # Step 1: Quantulum3 extracts quantity + unit from text (dynamic)
        quantity = extract_first_quantity(value_str)

        if not quantity:
            # No unit detected — try as plain number
            try:
        ...
        # Step 2: Pint converts to SI base unit (dynamic)
        ...
        # Fallback: Quantulum extracted it but Pint can't convert
        # (e.g., custom/unknown unit) — return as-is
        return {
