...
Responsibilities:
- Qdrant vector search with payload filters (PRIMARY)
- SQL filtering via Supabase (OPTIONAL, disabled by default)
- Return candidate listing_ids ONLY

IMPORTANT DESIGN DECISION:
SQL filtering is now DISABLED by default (use_sql_filter=False).
Qdrant payload filters already handle intent + domain/category filtering.
The SQL filter was redundant and O(N) in Python.

Keep SQL filter available as optional fallback for:
- Debugging/comparison
- Complex filters not supported by Qdrant payloads
- Migration period
...
from src.services.embedding.service import build_embedding_text
from src.config.settings import settings
from src.config.clients import DatabaseClients
...
RetrievalClients = DatabaseClients  # Consolidated into src.config.clients
...
def retrieve_candidates(
    ...
    query_listing: Dict[str, Any],
    limit: int = DEFAULT_LIMIT,
    use_sql_filter: bool = False,  # DISABLED by default - Qdrant payload filters handle this
    verbose: bool = True
) -> List[str]:
    """
    ...
    Pipeline:
    1. SQL filter (Supabase) - OPTIONAL, disabled by default
    2. Qdrant vector search with payload filters (handles intent + domain/category)
    3. Return candidate listing_ids

    IMPORTANT: SQL filtering is disabled by default because:
    - Qdrant payload filters already handle intent + domain/category filtering
    - SQL filter was O(N) in Python (fetched all rows, filtered in memory)
    - Redundant work that added latency without improving quality

    Enable SQL filter only if you need:
    - Complex filters not supported by Qdrant payloads
    - Debugging/comparison with Qdrant results
    - Additional filtering beyond intent + domain/category
    ...
    Args:
        ...
        query_listing: Normalized query listing
        limit: Number of candidates to return
        use_sql_filter: Whether to apply SQL filtering first (default: False)
